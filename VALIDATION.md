...
## Seção de candidaturas favoritas

Foi adicionada a rota **Favoritos** na navegação principal, com consulta isolada por usuário que lista as candidaturas marcadas mesmo quando pertencem a coletas anteriores. A seção permite pesquisar por nome, cargo ou partido, filtrar por estágio comercial, visualizar último contato e próximo follow-up, remover o favorito e abrir o perfil com histórico. A conferência visual foi realizada com dados reais de favoritos em desktop e em viewport móvel de 375 px. O teste DOM `ElectionFavorites.test.tsx` comprova separadamente a lista vazia sem filtros — com atalho para a Base Eleitoral — e a busca filtrada sem resultados, que não oferece esse atalho. A validação automatizada passou com **31 arquivos e 64 testes**, além de `pnpm check` e `pnpm build`.
